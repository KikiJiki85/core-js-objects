...
function compareObjects(obj1, obj2) {
  const obj1Strnig = JSON.stringify(obj1);
  const obj2String = JSON.stringify(obj2);
  return obj1Strnig === obj2String;
}
...
function makeWord(lettersObject) {
  const result = [];
  const newObj = { ...lettersObject };
  Object.keys(newObj).forEach((key) => {
    newObj[key].forEach((idx) => {
      result[idx] = key;
      return result;
    });
  });
...
function fromJSON(proto, json) {
  ...
  return Object.assign(newObj, parsedObject);
}
